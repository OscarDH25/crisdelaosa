...
@section('content')
    <div class="form-container">
        ...
        <p class="sign-up-label">Ingresa tus datos para acceder a tu cuenta.</p>
        <br>

        <form class="form" method="POST" action="{{ route('login.comprobar') }}">
            @csrf

            <input type="email" class="input {{ $errors->has('email') ? 'input-error' : '' }}"
                placeholder="Correo Electrónico" name="email" value="{{ old('email') }}" required autofocus>
            @if ($errors->has('email'))
                <div class="error">{{ $errors->first('email') }}</div>
            @endif

            <input type="password" class="input {{ $errors->has('password') ? 'input-error' : '' }}"
        ...
        <p class="sign-up-label">
            ¿No tienes una cuenta? <a href="{{ route('signIn') }}" class="sign-up-link">Crear una cuenta</a>
        </p>
